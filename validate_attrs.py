...
from query_abstract import NameCheck

# TRY TO PUT h2h AND EXCLUDE AT THE END OF THE ATTR CHECKING PROCESS
def generate_url(name, tour, attrs={}):
    '''
    Create the matching URL for a specific query to a player's match data page
    on Tennis Abstract by translating the user's chosen name and attributes to
    the format recognized by the website.
    ...
    attrs : dict, optional
        ...
        are allowed; **see THE DOCUMENTATION** for a full accounting.
    '''
    ...
    # check whether there's a matching name on the site
    url_name = NameCheck(name, tour).name_str

    # if so, add the proper form
    query_url += url_name

    # then, add query (or ask for whole career data if attrs is empty)
    query_url += _validate_attrs(tour, **attrs)

    return query_url

def _validate_attrs(tour, **kwargs):
    '''
    Validates the keys and values provided by the user in self.generate_url()'s
    `attrs` argument.
    ...
    **kwargs : optional
    ...
    for key, val in kwargs.items():
        ...
        if (key == H2H_KEY) or (key == EXCLUDE_KEY):
            ...
            if type(val) == str:
                surf_code = _validate_surfaces(val)
            elif type(val) == list:
                surfaces = [_validate_surfaces(sf) for sf in val]
            ...
            if type(val) == str:
                lvl_code = _validate_levels(val, tour)
            elif type(val) == list:
                levels = [_validate_levels(lv, tour) for lv in val]
            ...
            if type(val) == str:
                event_code = _validate_events(val, tour)
            elif type(val) == list:
                events = [_validate_events(ev, tour) for ev in val]
            ...
            if type(val) == str:
                round_code = _validate_round(val, tour)
            elif type(val) == list:
                rounds = [_validate_round(rd, tour) for rd in val]
                _validate_round_list(rounds)
            ...
            if type(val) == str:
                set_code = _validate_set(val, tour)
            elif type(val) == list:
                sets = [_validate_set(st, tour) for st in val]
                _validate_set_list(sets)
            ...
            score_code = _validate_score(val)
            attr_codes[key] = prefix + score_code

        elif key == ASRANK_KEY:
            prefix = 'G'

            asrank_code = _validate_asrank(val)
            attr_codes[key] = prefix + asrank_code

        elif key == VSRANK_KEY:
            prefix = 'I'

            vsrank_code = _validate_vsrank(val, tour)
            attr_codes[key] = prefix + vsrank_code

        elif key == VSCURR_KEY:
            prefix = 'R'

            vscurr_code = _validate_vscurrrank(val, tour)
            ...
            if type(val) == str:
                entry_code = _validate_entry(val, key)
            elif type(val) == list:
                entries = [_validate_entry(en, key) for en in val]
            ...
            #if type(val) == str:
            #    height_code = _validate_height(val, tour)
            #elif type(val) == list:
            #    heights = [_validate_height(h, tour) for h in val]
            #    #_validate_height_list(heights) # wrong behavior on site
...
def _validate_levels(value, tour):
    '''
    Called from self._validate_attrs().
...
def _simple_event_code(value, with_suffix=True):
    '''
    RENAME ME!
...
def _validate_events(value, tour):
    '''
    Called from self._validate_attrs().
...
def _validate_vsrank(value, tour):
    '''
    Called from self._validate_attrs().
    ...
    if type(value) == str:
        ...
        if value in {'Top 10', 'Top 20', 'Top 50', 'Top 100'}:
            ...
            if tour == 'WTA':
                return _validate_vsrank((1, 5), tour)
            else:
                code = _simple_event_code(value)
        else:
            raise ValueError(
        ...
        # ensure that ranks five digits or less and are ints (or is_integer=True for floats)
        prefix = 'cx'
        base_str = '10000'
...
def _validate_vscurrrank(value, tour):
    '''
    Called from self._validate_attrs().
...
def _validate_entry(value, key): # SWITCH ORDER OF ARGS?
    '''
    Called from self._validate_attrs().
    ...
    key : str, required
        Will either be 'as entry' or 'vs entry'. Needed since this method can
        handle either but needs to know which is pertinent when called.
    '''
    value = value.lower()
...
def _validate_height(value, tour):
    '''
    Called from self._validate_attrs().
...
def _validate_height_list(heights):
    '''
    Called from self._validate_attrs().
    ...
    if len(proposed_code) > 1 and proposed_code not in allowed_codes:
        # length 1 codes already checked in _validate_height
        raise ValueError(
...
def _validate_round(value, tour):
    '''
    Called from self._validate_attrs().
...
def _validate_round_list(rounds):
    '''
    Called from self._validate_attrs().
...
def _validate_set(value, tour):
    ...
    if (  (value == 'straight sets') or (value == 'straights')
        ...
        if tour == 'WTA': # this flag doesn't catch 1984-98's best of 5 wta finals matches
            raise ValueError(
